...
class OutputMessage:
    ...
    def from_content(cls, content: str):
        ...
        try:
            # Extract Reason
            reason_match = re.search(r'## Reason\s*(.*?)\s*## Response', content, re.DOTALL | re.IGNORECASE)
            if reason_match:
                reason = reason_match.group(1).strip()
            
            # Extract Response
            # Look for Response followed by Update Scenes OR Match Connection OR end of string
            response_match = re.search(r'## Response\s*(.*?)\s*(?:## Update(?:d)? Scenes|## Match(?:ed)? Connection|$)', content, re.DOTALL | re.IGNORECASE)
            if response_match:
                response = response_match.group(1).strip()
            
            # Extract Update Scenes
            updated_scenes_match = re.search(r'## Update(?:d)? Scenes\s*.*?```json\s*(.*?)\s*```', content, re.DOTALL | re.IGNORECASE)
                ...
                if json_str and json_str.lower() != "null":
                    try:
                        ...
                        if isinstance(scenes_data, list):
                            updated_scenes = [Scene.from_dict(scene_data) for scene_data in scenes_data]
                    except json.JSONDecodeError:
                        pass
            
            # Extract Match Connection
            match_connection_match = re.search(r'## Match(?:ed)? Connection\s*.*?```json\s*(.*?)\s*```', content, re.DOTALL | re.IGNORECASE)
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "match_connection": self.match_connection.to_dict() if self.match_connection else None,
            "reason": self.reason,
