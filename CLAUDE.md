...
**Name**: Pivot
**重要**: !!!本项目还没有上线，所以请你更多着眼于长远，不要写那些对过往逻辑需要兼容的冗余代码，而是着手写简洁、易读、逻辑清晰、可扩展性强的逻辑。如果你修改了db schemas可能导致程序异常，可以在结尾随时叫我删除一下数据库重启。如果你有一些拿不准的问题，请你先与我互动探讨解答，而不是着急上手写代码。!!!
**重要**: 所有与持久层数据交互（文件、数据库、缓存等）都需要通过service层，其他代码不要直接交互持久层数据，service都写在`server/app/service`下。请确保service的高复用性，尽量设计成简单的CRUD高可扩展操作，尽量不要写成一堆按用例来的定制service接口。
**重要**: 站在用户角度去考虑，如何能最简单、最清晰、最符合直觉地操作并得到结果。
