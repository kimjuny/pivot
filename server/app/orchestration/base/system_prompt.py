...
class SystemPrompt:
    ...
    def get_build_prompt(self, existing_agent: dict[str, Any] | None = None) -> dict[str, str]:
        ...
        if existing_agent:
            current_agent_config = json.dumps(existing_agent, indent=2, ensure_ascii=False)

        content = self._build_prompt_template.replace("{{current_agent_config}}", current_agent_config)
